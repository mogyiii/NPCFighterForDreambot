...
public class BotArea {
    ...
    private Area StartedArea = new Area();
    private Area WalkToArea;
    public BotArea(Factory factory) {
        _factory = factory;
    }
    public void setStartedArea(){
        StartedArea.generateArea(_factory.getMain().get_CombatVariables().get_WindowVariables().getAreaSize(), _factory.getMain().getLocalPlayer().getTile());
    }
